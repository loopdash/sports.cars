...
(function (root) {
  ...
  //
  // The year gate is authoritative: we only tag a generation when the
  // listing's year actually falls inside that generation's production window.
  // If a year is given and lands outside every window we know about, we
  // return null rather than guess — a 1992 911 (a 964/993, outside our
  // taxonomy) must NOT be mislabelled as a 992. Likewise a listing with no
  // year is left untagged rather than assigned the first-listed generation.
  function matchGeneration(rows, make, model, year) {
    ...
    if (!y) return null; // no reliable year → don't guess a generation
    var inYear = cands.filter(function (r) { return y >= r.yStart && y <= r.yEnd; });
    if (!inYear.length) return null; // year outside every known window → untagged
    // Narrowest window wins when a year lands in more than one candidate
    // (e.g. overlapping special-edition ranges), so the tightest match tags.
    inYear.sort(function (a, b) { return (a.yEnd - a.yStart) - (b.yEnd - b.yStart); });
    return inYear[0];
  }
