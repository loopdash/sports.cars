...
(function () {
  ...
  // Static build navigates to search.html; the WordPress theme sets
  // window.SC_SEARCH_URL to the /search/ permalink.
  ...
  function initFilterForm() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      var qs = p.toString();
      window.location.href = SEARCH_URL + (qs ? "?" + qs : "");
    });
  }
  ...
  /* ---------- Mobile nav ----------
     The concept3 nav has no hamburger in the markup, so build one here
     from the existing links + actions. Single-sourced: works on every
     page that loads this file, no per-page HTML changes. */
  function initMobileNav() {
    var nav = document.querySelector(".c3-nav");
    if (!nav || nav.querySelector(".c3-nav__burger")) return;
    var inner = nav.querySelector(".c3-nav__inner");
    var links = nav.querySelector(".c3-nav__links");
    var actions = nav.querySelector(".c3-nav__actions");
    if (!inner || !links) return;

    var burger = document.createElement("button");
    burger.className = "c3-nav__burger";
    burger.setAttribute("aria-label", "Menu");
    burger.setAttribute("aria-expanded", "false");
    burger.innerHTML =
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true">' +
      '<path class="c3-burger-open" d="M3 6h18M3 12h18M3 18h18"/>' +
      '<path class="c3-burger-close" d="M5 5l14 14M19 5L5 19" style="display:none"/></svg>';
    inner.appendChild(burger);

    var panel = document.createElement("div");
    panel.className = "c3-nav__mobile";
    var wrap = document.createElement("div");
    wrap.className = "c3__container";
    // clone the primary links, preserving the active state
    [].slice.call(links.querySelectorAll("a")).forEach(function (a) {
      wrap.appendChild(a.cloneNode(true));
    });
    if (actions) {
      var act = document.createElement("div");
      act.className = "c3-nav__mobile-actions";
      [].slice.call(actions.querySelectorAll("a")).forEach(function (a) {
        act.appendChild(a.cloneNode(true));
      });
    ...
    function close() {
      ...
      burger.setAttribute("aria-expanded", "false");
      burger.querySelector(".c3-burger-open").style.display = "";
      burger.querySelector(".c3-burger-close").style.display = "none";
    }
    burger.addEventListener("click", function () {
      var open = panel.classList.toggle("is-open");
      burger.setAttribute("aria-expanded", String(open));
      burger.querySelector(".c3-burger-open").style.display = open ? "none" : "";
      burger.querySelector(".c3-burger-close").style.display = open ? "" : "none";
    });
  ...
  /* ---------- Analytics loader (once per page, any entry script) ---------- */
  function loadAnalytics() {
    if (window.__scAnalyticsLoaded) return;
    window.__scAnalyticsLoaded = true;
    var s = document.createElement("script");
    // window.SC_ASSETS is set by the WordPress theme; falls back to the
    // relative path when running as the static build.
    s.src = (window.SC_ASSETS ? window.SC_ASSETS + "/js" : "assets/js") + "/analytics.js";
    s.defer = true;
