...
(function () {
  ...
  // API base: the Node dev proxy serves /api/*; the WordPress theme sets
  // window.SC_API_BASE to its REST namespace. Same request/response contract.
  var API = (typeof window !== "undefined" && window.SC_API_BASE) ? window.SC_API_BASE : "/api";
  // Page URLs differ between the static build (*.html) and WordPress
  // permalinks (/search/, /listing/). The theme sets these globals.
  var LISTING_URL = (typeof window !== "undefined" && window.SC_LISTING_URL) ? window.SC_LISTING_URL : "listing.html";
  var SEARCH_URL  = (typeof window !== "undefined" && window.SC_SEARCH_URL)  ? window.SC_SEARCH_URL  : "search.html";
  function withQuery(base, qs) { return base + (base.indexOf("?") >= 0 ? "&" : "?") + qs; }
  ...
  function cardHTML(v) {
    var link = v.id ? withQuery(LISTING_URL, "id=" + encodeURIComponent(v.id)) : LISTING_URL;
    var media = v.photo
    ...
    fetch(API + "/dealer?dealer_id=" + encodeURIComponent(dealerId) + "&rows=24")
      ...
      .then(function (data) {
        ...
        if (chipsEl) {
          ...
          b.textContent = "×";
          b.addEventListener("click", function () { location.href = SEARCH_URL; });
          chip.appendChild(b);
          chipsEl.appendChild(chip);
        }
      })
  ...
  function initDealerPage() {
    ...
    if (viewAll) viewAll.setAttribute("href", withQuery(SEARCH_URL, "dealer_id=" + encodeURIComponent(id)));
    ...
    fetch(API + "/dealer?dealer_id=" + encodeURIComponent(id) + "&rows=9")
      ...
      .then(function (data) {
        ...
        if (!list.length) {
          // Count known but bodies unavailable (MarketCheck dealer-body gap):
          // point buyers at the dealer-scoped search rather than a stuck grid.
          grid.innerHTML =
            '<p style="grid-column:1/-1;color:var(--c3-dim)">Browse this dealer\'s ' +
            (data.num_found ? data.num_found.toLocaleString("en-US") + " " : "") +
            'vehicles in <a href="' + withQuery(SEARCH_URL, "dealer_id=" + encodeURIComponent(id)) +
            '" style="color:var(--c3-lift);border-bottom:1px solid var(--c3-line)">search</a>.</p>';
          return;
        }
  ...
  function initListingPage() {
    ...
    fetch(API + "/listing/" + encodeURIComponent(id))
      ...
      .then(function (v) {
        ...
        if (sellerName && v.dealer) sellerName.textContent = v.dealer;
        var sellerMeta = document.querySelector("[data-seller-meta]");
        if (sellerMeta) {
          sellerMeta.textContent = v.location
            ? v.dealer ? v.location : "Location: " + v.location
            : (v.seller_type ? v.seller_type + " seller" : "");
        }

        /* ---- factual badges (no verified/history claims) ---- */
        var badges = document.querySelector("[data-listing-badges]");
        if (badges) {
          var bits = [];
          if (v.body_type) bits.push(v.body_type);
          if (v.transmission) bits.push(v.transmission);
          if (v.drivetrain) bits.push(v.drivetrain);
          if (typeof v.miles === "number") bits.push(milesLabel(v.miles));
          badges.innerHTML = bits.map(function (b) {
            return '<span class="listing-badge">' + esc(b) + "</span>";
          }).join("");
        }

        /* ---- documentation card: everything here is sourced ---- */
        var vinBadge = document.querySelector("[data-vin-badge]");
        if (vinBadge) vinBadge.textContent = "VIN: " + (v.vin || "Not provided");
        var docVin = document.querySelector("[data-doc-vin]");
        if (docVin) docVin.textContent = v.vin ? "VIN " + v.vin : "VIN not provided";
        var docListed = document.querySelector("[data-doc-listed]");
        if (docListed && typeof v.dom === "number") {
          docListed.textContent = v.dom + (v.dom === 1 ? " day listed" : " days listed");
        }
        var docSpec = document.querySelector("[data-doc-spec]");
        if (docSpec) docSpec.textContent = v.generation ? v.generation : "Spec on file";
        var docSeller = document.querySelector("[data-doc-seller]");
        if (docSeller && v.dealer) docSeller.textContent = v.dealer;
        // Source / original-listing links point at the dealer's VDP.
        var srcLink = document.querySelector("[data-source-link]");
        if (srcLink) {
          if (v.vdp_url) { srcLink.href = v.vdp_url; srcLink.target = "_blank"; srcLink.rel = "noopener"; }
          else { srcLink.style.display = "none"; }
        }

        /* ---- neutral, fact-only summary (no ownership/title claims) ---- */
        var about = document.querySelector("[data-about]");
        if (about) {
          var s1 = "This " + (v.title || "vehicle");
          var facts = [];
          if (typeof v.miles === "number") facts.push("shows " + milesLabel(v.miles));
          if (v.exterior) facts.push(v.interior ? v.exterior + " over " + v.interior : "finished in " + v.exterior);
          else if (v.interior) facts.push(v.interior + " interior");
          if (v.engine) facts.push("a " + v.engine + " engine");
          if (v.transmission) facts.push(v.transmission + " transmission");
          s1 += facts.length ? " " + facts.join(", ") + "." : ".";
          var s2 = v.dealer
            ? "Listed by " + v.dealer + (v.location ? " in " + v.location : "") + ". Contact the dealer for full history and documentation."
            : "Contact the seller for full history and documentation.";
          about.innerHTML = "<p>" + esc(s1) + "</p><p>" + esc(s2) + "</p>";
        }

        /* ---- keep Vehicle JSON-LD in sync with the live listing ---- */
        var ld = document.getElementById("vehicle-jsonld");
        if (ld) {
          var DRIVE = {
          ...
          var obj = { "@context": "https://schema.org", "@type": "Vehicle", name: v.title || "Vehicle" };
          if (v.make) obj.manufacturer = { "@type": "Organization", name: v.make };
          if (v.model) obj.model = v.model;
          if (v.year) obj.vehicleModelDate = String(v.year);
          if (v.vin) obj.vehicleIdentificationNumber = v.vin;
          if (v.body_type) obj.bodyType = v.body_type;
          if (v.transmission) obj.vehicleTransmission = v.transmission;
          if (v.exterior) obj.color = v.exterior;
          if (v.drivetrain && DRIVE[v.drivetrain]) obj.driveWheelConfiguration = DRIVE[v.drivetrain];
          if (typeof v.miles === "number") {
            obj.mileageFromOdometer = { "@type": "QuantitativeValue", value: v.miles, unitCode: "SMI" };
          }
          if (typeof v.price === "number") {
            obj.offers = { "@type": "Offer", price: v.price, priceCurrency: "USD", availability: "https://schema.org/InStock" };
            if (v.vdp_url) obj.offers.url = v.vdp_url;
          }
          try { ld.textContent = JSON.stringify(obj, null, 2); } catch (e) { /* leave static */ }
        }
