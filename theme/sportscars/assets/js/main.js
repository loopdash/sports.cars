...
(function () {
  ...
  var PARTIALS = {
    header:
      '<header class="nav"><div class="nav__inner">' +
        '<a href="index.html" class="nav__logo" aria-label="Sports.Cars home"><b>SPORTS</b><span class="logo-dot">.</span><span>CARS</span></a>' +
      ...
      '<footer class="footer"><div class="container">' +
        '<div class="footer__top">' +
          '<div class="footer__brand">' +
            '<div class="footer__brand-logo"><b>SPORTS</b><span class="logo-dot">.</span><span>CARS</span></div>' +
  ...
  /* ---------- Hero search: hand off to the search page ----------
     Builds a query string from the hero fields and navigates to
     search.html, which reads it back and runs the filters. */
  function initFilters() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      var make = form.querySelector("#f-make");
      var model = form.querySelector("#f-model");
      var price = form.querySelector("#f-price");
      var body = form.querySelector("#f-body");
      function chosen(sel) { return sel && sel.selectedIndex > 0 ? sel.value.trim() : ""; }

      var p = new URLSearchParams();
      if (chosen(make)) p.set("make", make.value);
      // Hero model options are trim-specific (e.g. "488 Pista") — pass as a
      // keyword so the search page matches on model + generation.
      if (chosen(model)) p.set("keyword", model.value);
      if (chosen(body)) p.set("body_type", body.value);
      var pv = chosen(price);
      if (pv) {
        if (/under/i.test(pv)) p.set("price_max", "100000");
        else if (/250k\+/i.test(pv)) p.set("price_min", "250000");
        else if (/100k/i.test(pv) && /250k/i.test(pv)) { p.set("price_min", "100000"); p.set("price_max", "250000"); }
      }
      var qs = p.toString();
      var searchUrl = (typeof window !== "undefined" && window.SC_SEARCH_URL) ? window.SC_SEARCH_URL : "search.html";
      window.location.href = searchUrl + (qs ? "?" + qs : "");
    });
  }
